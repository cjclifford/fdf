...
/*   Updated: 2018/08/09 16:19:36 by ccliffor         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

static int	tran_x(t_param param, float i)
{
...
static int	tran_y(t_param param, float i)
{
...
void		draw_map(t_param *param)
{
	...
	while (y < param->size_z)
	{
		...
		while (x < param->size_x)
		{
			...
			if (y + 1 < param->size_z)
			{
				line[0] = tran_x(*param, param->points[i][0]);
				line[1] = param->height - tran_y(*param, param->points[i][1]);
				line[2] = tran_x(*param, param->points[i + param->size_x][0]);
				line[3] = param->height - tran_y(*param, param->points[i + 
					param->size_x][1]);
			...
			if (x + 1 < param->size_x)
			{
				line[0] = tran_x(*param, param->points[i][0]);
				line[1] = param->height - tran_y(*param, param->points[i][1]);
				line[2] = tran_x(*param, param->points[i + 1][0]);
				line[3] = param->height - tran_y(*param, param->points[i + 
					1][1]);
				line_put_image(param, line, i);
			}
